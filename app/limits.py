...
# A deployment may raise its own callers' allowances: `fn(rule, request, limit)`
# returns the limit to use. Off by default, so the open source project has one
# set of numbers and they are the ones in RULES above. It can only raise.
_allowance_hook = None


def set_allowance_hook(fn) -> None:
    global _allowance_hook
    _allowance_hook = fn
...
def check(rule: str, request) -> tuple[bool, int, dict]:
    ...
    if _allowance_hook is not None:
        try:
            limit = max(limit, int(_allowance_hook(rule, request, limit)))
        except Exception:
            pass                                # a broken hook must not refuse anyone
